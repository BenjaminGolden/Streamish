...
import { useHistory } from 'react-router-dom';
 ...
 const VideoForm = ({getVideos}) => {
    ...
    //the handle save function calls the addVideo fetch call, which communicates with the video controller which calls the  video repository, which calls the DB, on the server side. the method in the repository communicates with the database to save the selected object properties. 
    const handleClickSaveVideo = (evt) => {
        ...
        else
        {
            ...
            // after the video is saved and the fields have been cleared, we call the getVideos function, which has been passed in as a prop, to automatically refresh the page and update state which causes a rerender, which adds the new video to the list without the need for the user to refresh the page. 
            // .then(() => getVideos())
