...
const Video = ({ video, name }) => {
  return (
    <Card >
      {video.userProfile != undefined ? <p className="text-left px-2">Posted by: <Link to={`/user/${video.userProfile.id}`}>{video.userProfile.name}</Link></p>: <p className="text-left px-2">Posted by: {name}</p>}
      <CardBody>
